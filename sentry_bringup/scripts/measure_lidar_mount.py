...
的 gravity 参数。URDF 里 lidar_joint 的旋转是 Rx(r)，所以重力在雷达系里
只有 [0, -n·sin r, -n·cos r] 两个分量，倾角 r = atan2(a_y, a_z)。

用法：车停在水平地面上、完全静止，起好驱动（跑整个 bringup 或者只跑 mid360_driver），
然后

    python3 src/sentry_bringup/scripts/measure_lidar_mount.py

只想看某个角度对应的参数、不连雷达：

    python3 src/sentry_bringup/scripts/measure_lidar_mount.py --tilt 30
...
- rpy 里的 yaw(0) 绕重力方向转，重力完全测不到，只能靠机械装配保证，
  或者对着已知墙面用点云标。
"""
...
JOINT_XYZ = (0.00, -0.15, 0.11)
LIDAR_HEIGHT = 0.21
FOV_MIN_DEG, FOV_MAX_DEG = -7.0, 52.0
CURRENT_NORM = 0.995636
URDF_REL_PATH = "sentry_description/urdf/mas2027_sentry.urdf"

def blind_center(tilt_rad):
    """base_link 原点在 lidar_link 中的坐标 = -Rᵀ·t，R = Rx(r)。"""
    tx, ty, tz = JOINT_XYZ
    c, s = math.cos(tilt_rad), math.sin(tilt_rad)
    return (-tx, -(ty * c - tz * s), -(ty * s + tz * c))
...
def report(tilt_deg, gravity):
    ...
    print('    <origin xyz="%.2f %.2f %.2f" rpy="%.4f 0 0"/>' % (*JOINT_XYZ, tilt))
    print("  sentry_hardware/Odometry/small_point_lio/config/mid360.yaml")
    print("    gravity: [ %.6f, %.6f, %.6f ]" % gravity)
    check = "对" if abs(blind_norm - 0.1780) < 5e-4 else "不对，JOINT_XYZ 和 URDF 不一致了"
    print("    # |blind_center| = %.4f，跟倾角无关，恒等于 0.1780 → %s" % (blind_norm, check))
